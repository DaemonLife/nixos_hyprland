...
# Определяем переменную с содержимым
let
  gimp_css = with config.lib.stylix.colors; ''
    /* GIMP theme css file */
    ...
    * {
    ...
    @define-color fg-color               rgb(200,200,200);
    @define-color bg-color               #${base00};
    @define-color border-color           rgb(16,16,16);
    ...
    /*
     * Color variable: 'sap'
     * A separator color used for UI elements
     * Used for visual separation between different interface components
     */
    @define-color sap            rgb(16,16,16);




    /* Border defines the boundaries around interface components */
    @define-color border            rgb(30,30,30);


    /* Color for canvas, where images are shown */
    @define-color canva            rgb(30,30,30);


    /**
     * Default color value for slider elements in the interface
     */
    @define-color slider         rgb(55,55,55);


    /**
     * 2nd Default color value for the main interface
     */
    @define-color fg2-color     rgb(150,150,150);


    @define-color widget-bg-color        rgb(30,30,30);
     ...
     */
    @define-color selected-color         rgb(64,64,64);
     ...
     * lighter on a dark theme.
     */
    @define-color extreme-selected-color rgb(64,64,64);

    /********* Variants for border colors *********/

    @define-color strong-border-color    rgb(20,20,20);
    ...
    @define-color scrollbar-slider-color rgba(91,91,91, 1.0);
    @define-color scrollbar-trough-color rgba(67,67,67, 1.0);

    @define-color ruler-color            rgb(45,45,45);
